...
function CallButton({ onClick }: { onClick(): void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full py-4 rounded-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-semibold text-lg cursor-pointer transition-colors"
    >
      Позвонить Чарли
    </button>
  );
...
export function Landing({ navigate }: LandingProps) {
  ...
  return (
    <div className="min-h-dvh w-full bg-slate-950 text-slate-100">
      <div className="max-w-md mx-auto px-5 py-10 flex flex-col gap-10">
        ...
        <footer className="text-xs opacity-50 pb-[env(safe-area-inset-bottom)] flex items-center gap-2 flex-wrap">
